...
```mermaid
flowchart LR
    H[Signed-in developer] -->|exact named lookup| L[Local child process]
...
The action exports environment variables only for the job lifetime, and the signer clears
its private key before any validation or upload step. Both Project CLI local runtimes and
manual deployment resolve each exact declared name with imports and recursion disabled;
failed lookup output is not returned to the caller. Server-side consumers read only the
...
5. Migrate legacy SSH credential rows to disabled state instead of guessing replacements.
   Production currently has zero private-network and access-route rows, so no live route is
   affected and no unused SSH key is injected.
...
- **A secret reaches logs or artifacts:** workflows avoid tracing and value enumeration;
  smoke checks validate names and public derivations only, and the signer clears its private
  key from the job environment before validation and upload.
