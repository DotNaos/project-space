...
workflows embed their own command lists. Git linked worktrees share a hook directory,
while the hook executes from the current worktree and reads its current configuration.
...
flowchart LR
    ...
    Local[Local CI preflight] --> Registry
