...
6. Implementation dispatch uses the canonical #763 operation, not a local
   worktree preparation followed by an untracked handoff. Candidate
   `cea348d649e01bc3bedbcba6b08ee7ae4001802f` carries model/reasoning and
   reporting-task fields, but reporting evidence remains caller-supplied. The
   local main/Manager/current-thread gate is the workflow proof and the exact
   landed contract remains a merge-readiness dependency.
