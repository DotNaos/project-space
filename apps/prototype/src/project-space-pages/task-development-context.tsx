...
function MachineCard({ expandedMachine, machine, onContinueDevelopment, onExpandedMachineChange, onOpenDevServer, onOpenPrototype, onThreadsOpenChange, threads, threadsOpen }: {
        ...
        <Disclosure.Content>
          <Disclosure.Body className="px-3 pb-2 pt-0">
            <div className="divide-y divide-current/[.07]">
              ...
              {threadsOpen ? <div className="space-y-1 bg-current/[.018] p-1.5">{threads.map((thread) => <button className="flex min-h-11 w-full items-center gap-2.5 rounded-xl px-2 py-2 text-left transition-colors hover:bg-current/[.045] hover:text-blue-300" key={thread.id} onClick={onContinueDevelopment} type="button"><span className="grid size-7 shrink-0 place-items-center rounded-full bg-current/[.055] text-current/35"><Bot className="size-3.5" /></span><span className="min-w-0 flex-1 truncate text-xs font-medium text-current/60">{thread.name}</span>{thread.status === "running" ? <LoaderCircle aria-label="Running" className="size-3.5 shrink-0 animate-spin text-emerald-300/80" /> : <Circle aria-label="Idle" className="size-2.5 shrink-0 text-current/25" />}</button>)}</div> : null}
