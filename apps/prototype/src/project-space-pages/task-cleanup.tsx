  ...
  return (
    <section className="mt-5 border-t border-current/[.08] pt-4" data-testid="closed-task-checkouts">
      ...
      <div className="mt-2 divide-y divide-current/[.07]">
        <CleanupRow
          detail={cleanup.remoteBranch === "deleted" ? "Deleted from GitHub" : "On GitHub · Safe to delete"}
          icon={GitBranch}
          label="Remote branch"
          secondary={task.branch ?? "Branch unavailable"}
        ...
        {cleanup.worktrees.map((worktree) => (
          <CleanupRow
            detail={worktree.safeToDelete
              ? "Local checkout · Clean · Safe to delete"
              : `Local checkout · ${worktree.uncommittedChanges} uncommitted · ${worktree.unstagedChanges} unstaged`}
            icon={Laptop}
            key={worktree.machine}
            label={worktree.machine}
            secondary={worktree.safeToDelete ? "No local changes" : "Review required before deleting"}
    ...
    <div className="flex min-h-12 items-center gap-3 py-1.5">
      ...
      <span className="min-w-0 flex-1">
        <span className="flex min-w-0 items-baseline justify-between gap-3">
          <span className="truncate text-xs font-medium text-current/70">{label}</span>
          <span className="truncate text-[10px] text-current/35">{secondary}</span>
        </span>
        <span className={`mt-0.5 flex items-center gap-1 text-[10px] leading-4 ${toneClass}`}>
