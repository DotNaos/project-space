...
| External latest checkpoint and macOS protected anchor | Prevent deletion, rollback, and replay of signed history | Integrity, availability |
...
The attacker cannot export the Secure Enclave key, complete LocalAuthentication without the owner, modify the signed helper's Data Protection Keychain item, or modify or roll back the separately administered runner gate and trust root. Portable runners additionally keep the external checkpoint outside repository-agent write and rollback authority.
...
| Protected anchor | signed native helper | CLI to macOS Data Protection Keychain | Signature-authorized compare-and-swap, no delete operation | `internal/approvalsigner/native/ProjectApprovalSigner.swift` |
...
8. Roll back history and checkpoint together → the helper-owned macOS anchor reports replay mismatch; portable runners deny the repository agent external-checkpoint rollback authority.
...
| TM-002 | Repository agent | Old valid history | Replay approval after signed revocation | False approval | Approval history | Signed event chain, external checkpoint, and macOS helper-owned monotonic anchor | Portable runner checkpoint authority | Keep portable checkpoint protected and report exact tip mismatch | Log accepted sequence/event digest | High | High | critical |
