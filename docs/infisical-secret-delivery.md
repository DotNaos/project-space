...
| Deployment membership and Tailscale inventory | `project-space-production` / `prod` | Production OIDC identity for automated deploys; signed-in human CLI session for manual deploys | Exact root-folder `PROJECT_SPACE_ALLOWED_EMAILS`, `TAILSCALE_OAUTH_CLIENT_ID`, and `TAILSCALE_OAUTH_CLIENT_SECRET` declarations; only the trusted backend receives them at runtime | Fail-closed member admission plus deployment-wide tailnet control-plane inventory; not a per-user connection or browser setting |
| Stored GitHub OAuth token encryption | `project-space-production` / `prod` | Production OIDC identity for automated deploys; signed-in human CLI session for manual deploys | Exact root-folder `PROJECT_SPACE_TOKEN_ENCRYPTION_KEY` declaration; only the trusted web service receives it | Implemented in #775 with an independently generated, stable Production key and safe legacy migration or explicit reconnect behavior |
...
| `project-space-production` | `prod` | Production deploy SSH/Tailscale, `PROJECT_SPACE_ALLOWED_EMAILS`, `TAILSCALE_OAUTH_CLIENT_ID`, `TAILSCALE_OAUTH_CLIENT_SECRET`, Clerk, GitHub OAuth application and stored-token encryption, release public key, rate-limit, MQTT provider, and Vite Clerk |
