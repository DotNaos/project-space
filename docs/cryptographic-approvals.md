...
The external checkpoint pins the latest accepted sequence, complete signed-event digest, operation, and content digest for each scope. Missing history while a checkpoint exists is `missing_history`; malformed or broken signed chains are `invalid_tampered`; restoring an older history, forking the tip, or rolling either side independently is `replay_checkpoint_mismatch`. Only an exact signed history/checkpoint match can be `approved` or `revoked`.
