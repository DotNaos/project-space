...
func allocateAgentName(
	...
	if err != nil {
		return agentNameResult{}, err
	}
	if dependencies.Registry == nil {
		return agentNameResult{}, agentNameCapabilityError(projectchat.ErrUnavailable)
	}
	...
	return agentNameResult{}, agentNameCapabilityError(err)
}

func isAgentNameOutage(err error) bool {
	return errors.Is(err, projectchat.ErrUnavailable) ||
...
func agentNameCapabilityError(err error) error {
	...
	case errors.Is(err, projectchat.ErrMissingCredential),
		errors.Is(err, projectchat.ErrMissingMachineID),
		errors.Is(err, projectchat.ErrInvalidCredential),
		errors.Is(err, projectchat.ErrInvalidMachineID):
		return fmt.Errorf("automatic agent naming needs a valid Project Connect machine registration; run project connect and retry: %w", err)
	case errors.Is(err, projectchat.ErrUnauthorized),
		errors.Is(err, projectchat.ErrNameRoleForbidden):
		return fmt.Errorf("Project Space rejected automatic agent naming; reconnect this machine with project connect and retry: %w", err)
	case errors.Is(err, projectchat.ErrNotRegistered):
		return fmt.Errorf("Project Space does not support the automatic agent-name capability; run project self-update --check and update Project Space before retrying: %w", err)
	case errors.Is(err, projectchat.ErrInvalidResponse),
		errors.Is(err, projectchat.ErrResponseTooLarge),
		errors.Is(err, projectchat.ErrRedirectRejected):
		return fmt.Errorf("Project Space returned an incompatible agent-name response; run project self-update --check and verify the server version before retrying: %w", err)
	case errors.Is(err, projectchat.ErrRateLimited):
		return fmt.Errorf("Project Space temporarily rate-limited automatic agent naming; retry shortly: %w", err)
	default:
		return err
	}
}
