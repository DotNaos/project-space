...
if [[ $# -ne 4 ]]; then
  echo "Usage: $0 <version> <source-sha> <codex-host> <runtime-output>" >&2
  exit 64
fi
...
assert_build_input "$codex_host" 157286400

/bin/mkdir -m 0700 "$runtime_output"
/usr/bin/install -m 0755 "$codex_host" "$runtime_output/project-codex-host"

codex_host_sha256=$(/usr/bin/shasum -a 256 "$runtime_output/project-codex-host")
codex_host_sha256=${codex_host_sha256%% *}
/usr/bin/printf '%s\n' \
...
/usr/bin/printf '%s  %s\n' \
  "$codex_host_sha256" project-codex-host \
