...
export function PrototypeReviewCodexDock({
  ...
  const conversation = state.conversations.find(
    (candidate) =>
      ...
      candidate.threadId === origin.threadId,
  );
  const machine = state.machines.find(
    (candidate) => candidate.id === origin.machineId,
  );
  ...
  const activeTurn =
    session?.status === "active" && Boolean(state.activeTurnId);
  const blockReason = connecting
    ...
    : session
      ...
      : (state.errorMessage ??
        "The verified Codex task is not reachable right now.");
  const isDark = theme === "dark";
  const hasMessage =
    Boolean(draft.trim()) || annotations.length > 0 || images.length > 0;
  const imageUploadPending = images.some((image) => image.status !== "ready");
  const working = connecting || activeTurn || Boolean(streamingAssistant);
  ...
  useEffect(() => {
    ...
    setFeedbackError(undefined);
    void controller
      .continue(origin, next.message, next.settings, next.imageAttachmentIds)
      .then(async (result) => {
      ...
      .catch((error) => {
        setFeedbackError(
          error instanceof Error
            ? error.message
            : "Could not send the queued Codex message.",
        );
      })
      .finally(() => {
  ...
  async function submit(event: FormEvent, settings?: CodexSessionTurnSettings) {
    ...
    try {
      ...
      setFeedbackError(
        error instanceof Error
          ? error.message
          : "Could not send prototype feedback.",
      );
    } finally {
      setSending(false);
    }
  ...
  function queueCurrentMessage(settings?: CodexSessionTurnSettings) {
    const comment =
      formatPrototypeFeedback(draft, annotations) ||
      (images.length ? "Please review the attached image." : undefined);
    if (!activeTurn || !comment || blockReason || sending || imageUploadPending)
      return;
    setQueuedMessages((current) => [
      ...current,
      {
        id: crypto.randomUUID(),
        imageAttachmentIds: images.flatMap((image) =>
          image.id ? [image.id] : [],
        ),
        message: comment,
  ...
  async function steerQueuedMessage(message: QueuedCodexMessage) {
    ...
    try {
      ...
      setFeedbackError(
        error instanceof Error
          ? error.message
          : "Could not steer the active Codex turn.",
      );
    } finally {
      setSending(false);
    }
  ...
  function removeQueuedMessage(message: QueuedCodexMessage) {
    ...
    message.imageAttachmentIds.forEach(
      (id) => void removePrototypeReviewCodexImage(id),
    );
    setQueuedMessages((current) =>
      current.filter((candidate) => candidate.id !== message.id),
    );
  ...
  async function updatePermissionProfile(permissionProfileId: string) {
    setFeedbackError(undefined);
    const result = await controller.updatePermissionProfile(
      origin,
      permissionProfileId,
    );
    const delivery = prototypeReviewCodexDelivery(result);
  ...
  return (
    <>
      <section
        ...
        data-prototype-codex-collapsed={collapsed ? "true" : undefined}
        data-prototype-dev-dock="true"
      >
        <div
          className={`grid min-w-0 items-end gap-2 ${
            annotationsEnabled
              ? "grid-cols-[3rem_minmax(0,1fr)] max-[640px]:grid-cols-[2.75rem_minmax(0,1fr)]"
              : "grid-cols-1"
          }`}
        >
          {annotationsEnabled ? (
          ...
          <CodexComposer
            ...
            compactTrailingContent={
              <button
                aria-label={
                  collapsed
                    ? "Expand Codex live chat"
                    : "Collapse Codex live chat"
                }
              ...
              >
                <ChevronDown
                  className={`size-4 transition-transform duration-300 ease-[cubic-bezier(0.2,0,0,1)] ${
                    collapsed ? "-rotate-90" : ""
                  }`}
                />
              </button>
            ...
            headerContent={
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="flex shrink-0 items-center">
                  <button
                    aria-label="Open full Codex chat"
                    className={`grid size-7 place-items-center rounded-lg transition-colors active:scale-[0.96] ${
                      isDark
                        ? "text-neutral-500 hover:bg-neutral-800 hover:text-neutral-100"
                        : "text-neutral-500 hover:bg-stone-200 hover:text-neutral-900"
                    }`}
                    title="Open full Codex chat"
                    type="button"
            ...
            leadingContentCollapsed={collapsed}
            leadingContent={
              streamingAssistant && !historyOpen ? (
                <div className="min-w-0">
                  <div className="flex min-w-0 items-start">
                    ...
                    />
                    <Loader2 className="ml-3 mt-1 size-3.5 shrink-0 animate-spin text-emerald-500" />
                  </div>
                ...
                <div className="w-full min-w-0">
                  <CodexComposer
                    ...
                    onSteerQueued={(message) =>
                      void steerQueuedMessage(message)
                    }
