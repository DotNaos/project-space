...
const statusTone: Record<RoadmapIssueAvailability, string> = {
  ...
  ready: 'border-emerald-500/80 text-emerald-300',
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected
          ? 'ring-2 ring-current/35 shadow-xl shadow-black/35'
    ...
    >
      <span className="flex min-w-0 items-start justify-between gap-2">
        <span className="inline-flex min-w-0 items-center gap-1.5">
          ...
          <Text className="truncate font-mono text-[11px] text-neutral-400">
        ...
        <Text className="shrink-0 rounded border border-current/30 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-[0.08em]">
          {roadmapStatusLabel[node.issue.availability]}
        </Text>
      </span>
      ...
      <span className="mt-auto flex min-w-0 items-end justify-between gap-2 pt-2">
        {node.planPosition ? (
          <Text className="rounded border border-neutral-700 bg-neutral-900 px-1.5 py-0.5 font-mono text-[10px] tabular-nums text-neutral-400">
