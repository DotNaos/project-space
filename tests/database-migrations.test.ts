...
} from '../server/database/tailscale-host-assignment-migration';
...
describe('database migrations', () => {
  test('separates Tailscale observations, classifications, and safe audit history', () => {
    ...
    expect(databaseMigrations.find(({ id }) => id === tailscaleInventoryMigrationId)?.id)
      .toBe(tailscaleInventoryMigrationId);
  });
  ...
  test('fails closed on retired 1Password references and permits only environment names', () => {
    ...
    expect(databaseMigrations.find(({ id }) => id === infisicalCredentialReferencesMigrationId)?.id)
      .toBe(infisicalCredentialReferencesMigrationId);
  });
  ...
  test('adds account-scoped Tailscale provider credentials without auditing secrets', () => {
    ...
    expect(databaseMigrations.find(({ id }) => id === tailscaleProviderConnectionMigrationId)?.id)
      .toBe(tailscaleProviderConnectionMigrationId);
  });
  ...
  test('retires and clears every legacy per-user Tailscale credential', () => {
    ...
    expect(databaseMigrations.find(({ id }) => id === tailscaleProviderConnectionRetirementMigrationId)?.id)
      .toBe(tailscaleProviderConnectionRetirementMigrationId);
  });
  ...
  test('reconciles only equivalent same-scope built-ins and preserves ambiguity', () => {
    ...
    expect(databaseMigrations.find(({ id }) => id === environmentDefinitionReconciliationMigrationId)?.id)
      .toBe(environmentDefinitionReconciliationMigrationId);
  });
  ...
  test('backfills only one owner-scoped Tailscale Environment to one exact Host', () => {
    ...
    expect(databaseMigrations.find(({ id }) => id === tailscaleEnvironmentOwnershipMigrationId)?.id)
      .toBe(tailscaleEnvironmentOwnershipMigrationId);
  });

  test('stores one owner-scoped manual Host assignment per Tailnet device', () => {
    expect(tailscaleHostAssignmentMigrationId).toBe('0060_tailscale_host_assignments');
    expect(tailscaleHostAssignmentMigrationSql).toContain(
    ...
    expect(tailscaleHostAssignmentMigrationSql).not.toMatch(/address|hostname|credential|token/i);
    expect(databaseMigrations.at(-1)?.id).toBe(tailscaleHostAssignmentMigrationId);
  });
  ...
  test('durably binds canonical Runtime control to one owner and exact Runtime session', () => {
    ...
    expect(databaseMigrations.find(({ id }) => id === canonicalRuntimeControlMigrationId)?.id)
      .toBe(canonicalRuntimeControlMigrationId);
  });
  ...
  test('extends canonical control additively with safe mutation replay and ownership fences', () => {
    ...
    expect(databaseMigrations.find(({ id }) => id === canonicalRuntimeMutationMigrationId)?.id)
      .toBe(canonicalRuntimeMutationMigrationId);
  });

  test('stores only bounded Runtime presentation fields', () => {
    expect(databaseMigrations.find(({ id }) => id === workspaceRuntimePresentationMigrationId)?.id)
      .toBe(workspaceRuntimePresentationMigrationId);
  ...
  test('defines the multi-user tables and their ownership constraints', () => {
    expect(databaseMigrations.map((migration) => migration.id)).toEqual([
      ...
      '0060_tailscale_host_assignments'
    ]);
