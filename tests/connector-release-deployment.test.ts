...
describe('connector release and production deployment contract', () => {
  test('pins the next immutable semantic release consistently', async () => {
    ...
    expect(buildInfo).toContain("const developmentVersion = '0.4.8';");
    expect(windowsPackaging).toContain("$version = '0.4.8'");
    expect(windowsPackaging).toContain('/releases/download/v0.4.8/');
    expect(windowsDocumentation).toContain('DotNaos\\Project\\0.4.8');
