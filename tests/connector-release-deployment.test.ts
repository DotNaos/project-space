...
describe('connector release and production deployment contract', () => {
  test('pins the next immutable semantic release consistently', async () => {
    ...
    expect(packageJson.version).toBe('0.4.49');
    expect(buildInfo).toContain("const developmentVersion = '0.4.49';");
    expect(windowsPackaging).toContain("$version = '0.4.49'");
    expect(windowsPackaging).toContain('/releases/download/v0.4.49/');
    expect(windowsDocumentation).toContain('DotNaos\\Project\\0.4.49');
