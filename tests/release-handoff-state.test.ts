...
describe('durable delivery recovery decisions', () => {
  ...
  test('binds Production recovery to its requested exact commit title', () => {
    const exact = run({
      displayTitle: `Production · ${commit} · v1.2.3`,
      headBranch: 'main',
    });
    ...
    expect(exactProductionRuns([olderInput, exact], commit, '1.2.3')).toEqual([exact]);
    expect(exactProductionRuns([exact], commit, '1.2.4')).toEqual([]);
