...
describe('issue creation state', () => {
  ...
  test('only selects real labels from the current ready response', () => {
  ...
  test('preserves the portable draft and invalidates repository-specific state on switch', () => {
    let state = connectedDraft('DotNaos/first');
    state = reduce(
      state,
      {
        repositoryKey: 'DotNaos/first',
        requestId: 'labels-first',
        type: 'labels-load-started'
      },
      {
        labels: [{ name: 'bug' }],
  ...
  test('retains values after failure, permits retry, and resets only for the current success', () => {
    let state = connectedDraft();
    state = issueCreationReducer(state, {
      requestId: 'create-1',
