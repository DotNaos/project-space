...
test('folds path-selected documentation checks into one clear PR job', () => {
  ...
  expect(workflow).toContain('bun run ci:check -- docs-specs');
  ...
  expect(workflow).toContain('bun run ci:check -- cli-docs-contract generated-cli-docs');
  expect(workflow).toContain('bun run ci:check -- docs-dependencies docs-typecheck docs-build');
  expect(workflow).toContain('bun run ci:check -- mobile-dependencies mobile-build');
  expect(workflow.match(/bun run ci:check -- locked-root-dependencies/g)).toHaveLength(1);
  expect(workflow).not.toContain('run: bun install --frozen-lockfile');
